...
class Blowfish
{
    ...
    protected function split_block($block) {
        return [
            ($block >> 32) & 0xFFFFFFFF,
            $block & 0xFFFFFFFF
        ];
    ...
    protected function join_words($left, $right) {
        return ($left << 32) | $right;
    }

    private function F($x) {
        $h = $this->S[0][$x >> 24] + $this->S[1][($x >> 16) & 0xFF];
        $h ^= $this->S[2][($x >> 8) & 0xFF];
        $h += $this->S[3][$x & 0xFF];
        return $h & 0xFFFFFFFF;
    }

    private function blowfish_round($left, $right, $P, $S) {
        for ($i = 0; $i < 16; $i++) {
            $left ^= $P[$i];
            $right ^= $this->F($left);
    ...
    private function encrypt_block($block, $P, $S) {
        list($left, $right) = $this->split_block($block);
        list($left, $right) = $this->blowfish_round($left, $right, $P, $S);
    ...
    private function generate_subkeys($key, $P, $S) {
        ...
        $key_bytes = array_merge($key_bytes, array_slice($key_bytes, 0, 4 - ($key_length % 4)));
        $key_length = count($key_bytes);

        for ($i = 0; $i < 18; $i++) {
            $start = ($i * 4) % $key_length;
            $chunk = array_slice($key_bytes, $start, 4);

            if (count($chunk) < 4) {
                $chunk = array_merge($chunk, array_slice($key_bytes, 0, 4 - count($chunk)));
            }

            $chunk_value = ($chunk[0] << 24) | ($chunk[1] << 16) | ($chunk[2] << 8) | $chunk[3];
            $P[$i] ^= $chunk_value;
        }

        $block = 0;
        for ($i = 0; $i < 18; $i += 2) {
            $block = $this->encrypt_block($block, $P, $S);
            list($P[$i], $P[$i + 1]) = $this->split_block($block);
        }

        for ($i = 0; $i < 4; $i++) {
            for ($j = 0; $j < 256; $j += 2) {
                $block = $this->encrypt_block($block, $P, $S);
                list($S[$i][$j], $S[$i][$j + 1]) = $this->split_block($block);
            }
        }
    ...
    public function encryptFile($inputFile,$userId) {
        ...
        fwrite($outputHandle, pack('L', $id));

        while (($block = $this->readBlock($inputHandle)) !== false) {
            $needs_padding = strlen($block) < 8;
            if ($needs_padding) {
                $block = $this->pad($block, 8);
            }
            
            $padded_block = str_pad($block, 8, "\x00", STR_PAD_RIGHT);
            $block_int = unpack('J', strrev($padded_block))[1];
            
            $encrypted_block = $this->encrypt_block($block_int, $P, $S);
            fwrite($outputHandle, strrev(pack('J', $encrypted_block)));
        }
